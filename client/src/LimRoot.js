...
function LimRoot({scoreB, setScoreB, totalScoreB,setTotalScoreB, stScore, setStScore, mssg, setMessage, taskNumber, setTaskNumber}) {
   ...
   // const [a, setA] = useState([]);
    ...
    useEffect(() => {
      ...
      const saveData = async (a, d, n, s, stIndef, stG, stIndef2, stG1, stIndef3, stG2, stIndef4, stG3, stIndef5, taskNumber, stAns) => {
        try {
          const docRef = await addDoc(collection(db, '1_root1'), { 
          z: '\lim{x\to\infty}[\sqrt{ax^n+bx^{m}+c}-\sqrt{dx^n+kx^m+p}],  a<>d,  m=1',
          zz: 'g=infty   infty   sqrt(a)   0',
      ...
      React.useEffect(() => {
        if ((stAns!=="notyet")){    
          saveData(a, d, n, s, stIndef, stG, stIndef2, stG1, stIndef3, stG2, stIndef4, stG3, stIndef5, taskNumber, stAns)
        }
      }, [stAns])
