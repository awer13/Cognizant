...
function Sum_1({scoreB, setScoreB, totalScoreB,setTotalScoreB, stScore, setStScore, mssg, setMessage, taskNumber, setTaskNumber}) {
    const  q=4, eps = 0.001;
    ...
    const [texExpression,setTextExpression] = useState('\\sum\\limits_{k=0}^{\\infty}\\left(\\dfrac{b}{5}\\right)^k,   ')
    const [texExpression0,setTextExpression0] = useState(' ')
    
    useEffect(() => {
      katex.render(`\\sum\\limits_{k=0}^{\\infty}\\left(\\dfrac{${b}}{5}\\right)^k    `, containerRef.current); //tut skobki drugie i ${tut variable}
  ...
  */
    ...
    const saveCalc = async (b, x, stXterm, xTerm, stS, sum,  stAns, ans, stSubst, subst,  taskNumber) => {
      try {
        ...
        const docRef = await addDoc(collection(db, 'sum1test'), { 
        //const docRef = await addDoc(collection(db, '00_sum_1'), { //thepeng
        //const docRef = await addDoc(collection(db, '01_sum_1'), { //thepeng
        z: 'sum (b/5)^k,    k>=0',
        zz: 'variants N=1+4*i, sum_1',
        TASKNUMBER: taskNumber,
          B_k: [b, x],
          a_k_ST: stXterm,
          a_k: xTerm,
