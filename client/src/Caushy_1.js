...
function Caushy_1({scoreC, setScoreC, totalScoreC,setTotalScoreC, stScore, setStScore, mssg, setMessage, taskStatus, setStatus, taskNumber, setTaskNumber}) {
    const  q=4, eps = 0.001;
    var scoreC=Array(q).fill(0);
    var  xTerm, l, l0, ans, subst;
    const submax=2;
   ...
   // xTerm=((c*x-(c-1))/((c+1)*x-c))**x; // 

    const [texExpression,setTextExpression] = useState('\\sum\\limits_{k=1}^{\\infty}\\left[\\dfrac{ck-b}{dk-c}\\right]^{sk+p},   ')
    ...
    useEffect(() => {
      katex.render(`\\sum\\limits_{k=1}^{\\infty}\\left[\\dfrac{${c}k-${c-1}}{${c+1}k-${c}}\\right]^{${s}k+${s+1}}    `, containerRef.current); //tut skobki drugie i ${tut variable}
    ...
    const saveCalc = async (c,x, s, stXterm, xTerm, stL, l,  stAns, ans, stSubst, subst, taskNumber) => {
      try {
        //const docRef = await addDoc(collection(db, 'caushy1mycalc'), {
        //const docRef = await addDoc(collection(db, 'caushy138'), { 
      //const docRef = await addDoc(collection(db, '00_caushy_1'), { //thepeng
      //const docRef = await addDoc(collection(db, '01_caushy_1'), { //thepeng
      const docRef = await addDoc(collection(db, 'caushy1test'), { //thepeng
       z: 'sum [(ck-(c-1))/((c+1)k-c)]^[sk+(s+1)],    k>=1',
       zz: 'variants N=3+4*i, caushy_1',
    ...
    React.useEffect(() => {
      if ((stSubst!='undef')&(stAns!='undef')){
        saveCalc(c, x, s, stXterm, xTerm, stL, l, stAns, ans, stSubst, subst, taskNumber)
      }
    }, [stSubst,stAns])
//}, [c,x, stXterm, stL, stAns, stSubst])
    ...
    useEffect(() => {
        ...
        setC(myrandom(2,maxc+1));
        setS(myrandom(2,maxc-1));
        setX(myrandom(3,10));
    }, [])

    const onInputL = (event) => {
      var w = (event.target.value);
      if (w!=="infty") w = Number(event.target.value)
      setStL(w);
    }
   ...
   // xTerm=((c*x-(c-1))/((c+1)*x-c))**(2*x+3); // 
    xTerm=((c*x-(c-1))/((c+1)*x-c))**(s*x+(s+1)); // 
    l0=c/(c+1);
    l=l0**s;
    ans="сходится";
    subst="Коши";

const handleSubmitSerieCaushy_1 = (event) => {
    let count = 0;
...
return (
  ...
  <form onSubmit={handleSubmitSerieCaushy_1}>
  <Container>
        {/* <h3> 3-есеп </h3>  */}
        <h3> Задача-3 </h3> 
      </Container>
