...
function LimExp2({scoreC, setScoreC, totalScoreC,setTotalScoreC, stScore, setStScore, mssg, setMessage,  taskStatus, setStatus, taskNumber, setTaskNumber}) {
    const  eps = 0.001, qnum=5;
    var scoreC=Array(qnum).fill(0);
    var  g, g1, g2, g3, indef, k, c,b,a;
    var max=5, meth,it;
   
    //const [stAns, setstAns] = useState([]);
    const [stIndef, setstIndef] = useState("noinput");
    //const [letsave, setLetsave] = useState(["notyet"]);
   ...
   const [texExpression3,setTextExpression3] = useState('a. ~ (\\dfrac{\\infty}{\\infty})~~~~~~~ b.~     (\\dfrac{0}{0})~~~~~~~ c.~     (1^{\\infty})~~~~~~~ d.~     ({x\\to} 0 \\\ либо \\ {\\alpha(x)\\to} 0)')
   ...
   const [texExpression4,setTextExpression4] = useState('\\lim\\limits_{x \\to \\infty}\\dfrac{x+{a}}{x+p} ')
    ...
    useEffect(() => {
       katex.render(`\\lim\\limits_{x \\to \\infty}\\left(\\dfrac{${c}x+${a}}{${c}x+${p}}\\right)^{${b}x-${k}}    `, containerRef.current); //tut skobki drugie i ${tut variable}
    }, [texExpression,c,a,p,b,k]); // tut dobavil b, chtoby kogda b menyaetsya zapuskalas eta funkciya
  ...
  useEffect(() => {
    katex.render(`\\lim\\limits_{x \\to \\infty}\\dfrac{${c}x+${a}}{${c}x+${p}}   `, container4Ref.current); 
 }, [texExpression4,a,p,c]); 

   
 const saveData = async (c, a, p, b, d, meth, stG1, stIndef, stG,  stG2, stG3, taskNumber, stAns) => {
  try {
    const docRef = await addDoc(collection(db, '1_exp2'), { //thepeng
...
React.useEffect(() => {
  if ((stAns!=="notyet")){  
  //if (letsave==="saveit"){
    saveData(c, a, p, b, d, meth, stG1, stIndef, stG,  stG2, stG3, taskNumber, stAns)
  }
}, [stAns])
...
var met;
    ...
    switch(it){
        case 1:{
          meth="1-ый замечательный предел";
          met="firstl";
        }
        break;
        case 2:{
          meth="2-ой замечательный предел";
          met="secondl";
          //break;
        }
        break;
        case 3:{
          meth="правило Лопиталя";
          met="lopital";
        }
            break;
        case 4:{
          meth="эквивалентные функции";
          met="equiv";
        }
         break;
    }
  ...
  if (met==="firstl")
   ...
   console.log("g1=",g1, "stG=",stG1,  "scoreC0=", scoreC[0]); 
   console.log("indef=",indef, "stIndef=", stIndef, "scoreC1=", scoreC[1]);
    console.log("g=",g, "stG=",stG,  "scoreC2=", scoreC[2]);
    console.log("g2=",g2, "stG=",stG2,  "scoreC3=", scoreC[3]);
    console.log("g3=",g3, "stG=",stG3,  "scoreC4=", scoreC[4]);
...
return (
  ...
  <Container0>  
      ...
      <label>  g1: </label>
         ...
         <h4> 3.2. Укажите случай, когда для раскрытия неопределенности можно применить {meth}: </h4>
        ...
        <option value="infinfzz"> a и b </option>
        <option value="zerozero">  b </option>
        <option value="1^infty"> c </option>
        <option value="xto0"> d </option>
      </select>
          </Col>
        </Row> 
        </Container>
      ...
      <label>  g2: </label>
        ...
        <h4> 3.5.  Введите значение  <myh ref={container2Ref} /> : </h4>
      <Row gutter={20}>
      <label>  g3: </label>
          <Col span={4}>
            {" "}
