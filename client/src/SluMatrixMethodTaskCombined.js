...
export default function SluMatrixMethodTaskCombined({ taskNumber, onDone }) {
  ...
  const header = `\\text{Решить }A\\,x=b,\\; ${latexMatrix(A,'A')}\\;,\\; ${latexVec(b,'b')}`
  const setVec = (setter,i,v)=> setter(prev=>{const cp=prev.slice(); cp[i]=v; return cp})

  const verify = () => {
    const rows=[]
    const W={ det:20, sol:45, check:35 }
    rows.push({ key:1, label:'Определитель \\det(A)', w:W.det, gained: near(Number(detUser),detA)?W.det:0 })
    rows.push({ key:2, label:'Решение x', w:W.sol, gained: x.every((v,i)=>near(Number(xUser[i]),v))?W.sol:0 })
    rows.push({ key:3, label:'AX\\;\\text{(подстановка)}', w:W.check, gained: [0,1,2].every(i=>near(Number(AxUser[i]), b[i]))?W.check:0 })
    const scorePercent = rows.reduce((s,r)=>s+r.gained,0)
    onDone?.({ scorePercent, rows, klass:'slu-matrix-method', params:{}, A })
  }
  ...
  return (
    <Box>
      ...
      {step===1 && (
        <>
          <TitleKx textLatex={'\\text{Шаг 1 — }\\det(A)'} />
      ...
      {step===2 && (
        <>
          <TitleKx textLatex={'\\text{Шаг 2 — решение }(x = A^{-1}b)'} />
          <Row gutter={8} align="middle" style={{ marginBottom:8 }}>
      ...
      {step===3 && (
        <>
          <TitleKx textLatex={'\\text{Шаг 3 — проверка }AX=b'} />
          <Row gutter={8} align="middle" style={{ marginBottom:8 }}>
