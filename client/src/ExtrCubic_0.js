...
function ExtrCubic_0({scoreA, setScoreA, totalScoreA,setTotalScoreA, stScore, setStScore, mssg, setMessage, taskStatus, setStatus, taskNumber, setTaskNumber}) {
  const  q=10;
  ...
  //const [scoA, setScoA] = React.useState([]);
 ...
 // const [onoff, setOnoff] = React.useState([])
  ...
  useEffect(() => {
    ...
    katex.render(`z=x^3+${b}y^3-${c}xy+${p}    `, containerRef.current); //tut skobki drugie i ${tut variable}
  ...
  const saveCalc = async (a, b, c, p, spNum, n, x,y,spx, spy,  dzx, dzy, xx, dxx, xy, dxy, dyx, yy, dyy, stD, det, stAns, ans, taskNumber) => {
    try {
        //const docRef = await addDoc(collection(db, '00_ExtrCubic0'), { 
          const docRef = await addDoc(collection(db, '000_ExtrCubic0'), { //restversion
