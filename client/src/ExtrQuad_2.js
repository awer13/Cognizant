...
function ExtrQuad_2({scoreA, setScoreA, totalScoreA,setTotalScoreA,stScore, setStScore, mssg, setMessage,taskNumber }) {
 // function ExtrQuad_2({scoreA, setScoreA, totalScoreA,setTotalScoreA, stScore, setStScore, mssg, setMessage,  taskStatus, setStatus}) {
    ...
    const saveData = async (a, b, c, taskNumber) => {
      try {
        const docRef = await addDoc(collection(db, 'quad2mydata'), {
        //const docRef = await addDoc(collection(db, 'quad2stdata'), {//thepeng
    ...
    const saveCalc = async (n, x,y,spx, spy, dzx, dzy, dxx,dxy,dyx,dyy, stD, det, stAns, sol, taskNumber) => {
      try {
        const docRef = await addDoc(collection(db, 'quad2mycalc'), {
        //const docRef = await addDoc(collection(db, 'quad2stcalc'), { //thepeng
