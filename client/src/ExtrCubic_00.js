...
function ExtrCubic_0({scoreA, setScoreA, totalScoreA,setTotalScoreA, stScore, setStScore, mssg, setMessage, taskNumber, setTaskNumber}) {
    const  q=10, eps = 0.001;
   ...
   var stX=Array(2).fill(0),
    ...
    useEffect(() => {
          katex.render(`z=${a}x^3+${b}y^3-${c}xy+${p}    `, containerRef.current); //tut skobki drugie i ${tut variable}
    ...
    const saveData = async (a, b, c, p, taskNumber) => {
      try {
        //const docRef = await addDoc(collection(db, 'cubic0mydata'), {
        //const docRef = await addDoc(collection(db, 'cubic0data'), {nuclear eng
        const docRef = await addDoc(collection(db, '0_cubic0stdata'), { //thepeng
    ...
    const saveCalc = async (n, x,y,spx, spy, dzx, dzy, dxx,dxy,dyx,dyy, stD, det, stAns, ans, taskNumber) => {
      try {
        //const docRef = await addDoc(collection(db, 'cubic0mycalc'), {
        //const docRef = await addDoc(collection(db, 'cubic0calc'), { nuclear engineering
        const docRef = await addDoc(collection(db, '0_cubic0stcalc'), { //thepeng
    ...
    React.useEffect(() => {
      saveCalc(n, x,y,spx, spy,dzx, dzy, dxx,dxy,dyx,dyy, stD, det, stAns, ans, taskNumber)
    }, [mssg])
    ...
    const onInputNpoints = (event) => {
        ...
        setdYY(dyy);
        let stD = Array(nstationar).fill(0);
        setstD(stD);
        let stAns = Array(nstationar).fill("undefined");
        setstAns(stAns);
    }
   ...
   if (n===spNum) { //the case student's n=2
    ...
    for (let i = 0; i < n; i++){
      ...
      else if ((stX[0]===spx[1]) && (stY[0]===spy[1])) {//the case student's n=1, stat point=nonzero (spx,spy)
      scoreA[1]=scoreA[1]+10;
      }
...
for (let i = 0; i < n; i++){
      if (stZx[i]===0){
        scoreA[3]=scoreA[3]+5/n;
      } 
      if (stZy[i]===0){
        scoreA[4]=scoreA[4]+5/n;
      }
    }
    for (let i = 0; i < 11; i++){
