...
//import PolynomFrom from './PolynomFrom' //   1+4*i
...
//import Sum_3a from './Sum_3a' //   1+4*i
//import Sum_3b from './Sum_3b' //   1+4*i
...
function MainLimit() {
 ...
 // const [nbstatus, setNBstatus] = React.useState('notyet');
  ...
  const saveResults = async (scoreA, scoreB, scoreC, stScore, firstName, lastName, taskNumber,taskStatus) => {
    ...
    const docRef = await addDoc(collection(db, '11series'), {
    ...
    console.log('taskNumber: ', taskNumber)
      ...
      for (let i = 0; i < 10; i++) {
        if (taskNumber===1+4*i) { // vars 1, 5, 9, 13, 17, 21, 25, 29, 33, 37 
          ...
          setFirstTask("ncssr_2"); //    p/c   div
          setSecondTask("limroot") //    b/(b-1) [ser: (1/b)^k]   conv
  ...
  return (
    ...
    <tr>
      ...
      <th scope="col"> вопросы по 1-ой задаче   </th>
      <th scope="col"> оценка по 1-ой задаче  </th>
      <th scope="col"> вопросы по 2-ой задаче    </th>
      <th scope="col"> оценка по 2-ой задаче  </th>
      <th scope="col"> вопросы по 3-ей задаче    </th>
      <th scope="col"> оценка по 3-ей задаче  </th>
       </tr>
    
      </thead>
  <tbody>
    <tr>  <td> {submax/numq}</td>
    <th scope="row"> неопределенность  </th>
      <td>{scoreA[0]} </td>
      <th scope="row"> неопределенность </th>
      <td>{scoreB[0]} </td>
      <th scope="row"> неопределенность  </th>
    ...
    <td> {submax}  </td>
    <th scope="row"> %  </th>
      <td>{totalScoreA*100/submax} %  </td>
      <th scope="row">    </th>
      <td>{totalScoreB*100/submax}%  </td>
      <th scope="row">    </th>
      <td>{totalScoreC*100/submax}%  </td>
    </tr>
