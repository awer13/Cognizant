...
function Dlmbr_1({scoreC, setScoreC, totalScoreC,setTotalScoreC, stScore, setStScore, mssg, setMessage,  taskStatus, setStatus, taskNumber, setTaskNumber}) {
    const  q=4, eps = 0.001;
    ...
    useEffect(() => {
      katex.render(`\\sum\\limits_{k=1}^{\\infty}\\dfrac{${b}^k}{(${b+1}k)!}    `, containerRef.current); //tut skobki drugie i ${tut variable}
    ...
    const saveCalc = async (b, x, stXterm, xTerm, stL, l,  stAns, ans, subst, stSubst, taskNumber) => {
      try {
        //const docRef = await addDoc(collection(db, 'dlmbr1mycalc'), {
        const docRef = await addDoc(collection(db, 'dlmbr1test'), {
      // const docRef = await addDoc(collection(db, '00_dlmbr_1'), { //thepeng
      //const docRef = await addDoc(collection(db, '01_dlmbr_1'), { //thepeng
       z: 'sum b^k/[(b+1)k!],    k>=1',
       zz: 'variants N=2+4*i, dlmbr_1, l=0',
...
//}, [b,x, stXterm, stL, stAns, stSubst])
    ...
    xTerm=(b**x)/(myfactorial((b+1)*x)); // 
    l=0;
    ans="сходится";
    subst="Даламбер";

const handleSubmitSerieDlmbr_1 = (event) => {
    let count = 0;
...
return (
  ...
  <form onSubmit={handleSubmitSerieDlmbr_1}>
  <Container>
      {/* <h3> 3-есеп </h3>  */}
      <h3> Задача-3 </h3> 
      </Container>
