...
*/  
  ...
  const onInputIndef2 = (event) => {
    let count = 0;
    console.log('onInputIndef2 count=', count)
     ...
     // count += 1;
      for (let i = 0; i < stDipN; i++) { 
        stDer2[i] = !isNaN((event.target[count].value))
          ? Number(event.target[count].value)
          : 0;
          console.log("i=",i, "count=",count, "stDer2=",stDer2[i]);
        count += 1;
      }
...
<label>     m </label>
  ...
  <Container> 
    ...
    <h4> 6. Выберите  либо введите значение(я)  <myh ref={containerDiRef}/>: </h4>
          <MatrixRow > 
    ...
    <Col span={25}> 
        ...
        <Col span={2}>
          ...
          <h4> 7. Введите стационарные точки </h4>
  <MatrixRow > 
