...
function ExtrQuad_3({scoreA, setScoreA, totalScoreA,setTotalScoreA, stScore, setStScore, mssg, setMessage, taskNumber, setTaskNumber}) {
    ...
    const saveData = async (a, b, c, taskNumber) => {
      try {
        //const docRef = await addDoc(collection(db, 'quad3mydata'), {
        const docRef = await addDoc(collection(db, 'quad3stdata'), {//thepeng
    ...
    const saveCalc = async (n, x,y,spx, spy, dzx, dzy, dxx,dxy,dyx,dyy, stD, det, stAns, sol, taskNumber) => {
      try {
        //const docRef = await addDoc(collection(db, 'quad3mycalc'), {
        const docRef = await addDoc(collection(db, 'quad3stcalc'), { //thepeng
