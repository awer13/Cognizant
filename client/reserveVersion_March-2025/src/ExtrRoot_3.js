...
function ExtrRoot_3({score, setScore, totalScore,setTotalScore, stScore, setStScore, mssg, setMessage, taskStatus, setStatus,taskNumber, setTaskNumber}) {
    ...
    const saveData = async (a, b, c, p, taskNumber) => {
      try {
        const docRef = await addDoc(collection(db, 'root3mydata'), {
        //const docRef = await addDoc(collection(db, 'root3stdata'), {//thepeng
    ...
    const saveCalc = async (n, x,y,spx, spy, dzx, dzy, dxx,dxy,dyx,dyy, stD, det, stAns, sol, taskNumber) => {
      try {
        const docRef = await addDoc(collection(db, 'root3mycalc'), {
        //  const docRef = await addDoc(collection(db, 'root3stcalc'), { //thepeng
